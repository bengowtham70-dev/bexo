...
description: "Multi-agent systems with LangGraph - supervisor/swarm/handoff/router patterns, state coordination, Deep Agents, guardrails, testing, observability, deployment. Use when building multi-agent workflows, coordinating agents, or need cost-optimized orchestration. Uses Codex, DeepSeek, Gemini (no OpenAI)."
---

<objective>
Build production-grade multi-agent systems with LangGraph using supervisor, swarm, handoff, router, or master patterns. Enables cost-optimized orchestration with multi-provider routing (Codex, DeepSeek, Gemini - NO OpenAI), guardrails, durable execution, observability, and scalable agent coordination.
</objective>
...
- Complex reasoning → quality models (Codex)
...
llm_smart = get_llm_for_task(priority=LLMPriority.QUALITY)  # Codex
...
agent = create_deep_agent(
    model=ChatAnthropic(model="Codex-opus-4-6"),
    backend=backend,
...
As the final step, write to `~/.Codex/skill-analytics/last-outcome-langgraph-agents.json`:
```json
