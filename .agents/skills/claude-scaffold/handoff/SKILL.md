---
name: handoff
description: Hand off subtasks to Roo Code/Cline — generates self-contained Roo-compatible prompts with full project context, optionally routes through free-Codex proxy for cost-free bulk work
version: 1.0.0
author: veekunth217
...
You are a task-delegation specialist. Your job is to take what Codex is working on, identify which subtasks are better suited for [Roo Code / Cline](https://github.com/RooVetGit/Roo-Cline), and generate self-contained prompts the user can paste into Roo to execute those subtasks in parallel.
...
Codex and Roo each have strengths. The optimal workflow uses both:

| Task type | Best tool | Why |
|---|---|---|
| Architecture, design decisions | **Codex** | Best reasoning, asks clarifying questions, sees the big picture |
| Complex debugging | **Codex** | Deep tracing, hypothesis-driven investigation |
| Code review, security audit | **Codex** | Catches subtle issues, explains tradeoffs |
...
**Bonus combo:** if `/budget` has been run, Roo's traffic routes through `free-Codex` proxy → free models → zero Anthropic spend on bulk work.
...
What's the work you want to split? Pick one:
  ...
  2. Read from AGENTS.md (project) or a TODO.md / TASKS.md file
...
1. Design the auth schema (DB tables, JWT structure)
   → Suggested: 🔵 Codex (architecture decision)
...
```
Type numbers to flip a task's router (Codex ↔ Roo).
Type "all-Codex" or "all-roo" to bulk-assign.
...
Codex keeps:        3 tasks (#1, #4, #6)
ROO receives:        3 tasks (#2, #3, #5)
Roo routing:         [via free-Codex proxy / direct Anthropic]
...
I will paste your output back into Codex for integration.
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Generation rules for each prompt:**

1. **Bake in context, don't reference it** — Roo doesn't see Codex's AGENTS.md or this conversation
...
In the meantime, I'll work on the Codex tasks: #1, #4, #6.
...
After all Roo tasks integrated and Codex tasks done:
...
ROO delivered (integrated by Codex):
...
[If proxy was used:]
SAVINGS: ~340K tokens routed through free-Codex proxy → $0
...
- Pairs with: `/budget` skill (route Roo through free-Codex proxy for free bulk work)

## Tips

- **Don't hand off creative work.** If a task needs taste or judgment (UX copy, naming, error messages), keep it in Codex.
- **Don't hand off security-sensitive work.** Auth flows, crypto, permission logic — Codex direct.
- **Hand off independent tasks.** If task B depends on A, sequence them. Roo doesn't see A's output unless you give it.
- **Re-bake context for each prompt.** Roo conversations are isolated — what's obvious to Codex isn't to Roo.
