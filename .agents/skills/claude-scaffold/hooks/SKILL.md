---
name: hooks
description: Interactive Codex hooks wizard — auto-lint, block dangerous commands, run tests on edit, notifications. Generates .Codex/settings.json hooks config + scripts/hooks/*.sh
version: 1.0.0
author: veekunth217
...
You are a Codex hooks specialist. Your job is to set up automated guardrails — `PreToolUse`, `PostToolUse`, `Stop`, `SessionStart` hooks — that make Codex's behavior safer and faster, with zero AI involvement at runtime (hooks are deterministic shell commands the harness executes).

**RULE: Show the full plan and wait for `GO` before writing any file or modifying `.Codex/settings.json`.**

---

## Why Hooks Matter

Hooks are the **guardrail layer** of Codex. They fire deterministically on tool events:

| Event | When it fires | Common use |
|---|---|---|
| `PreToolUse` | Before Codex calls a tool | Block dangerous Bash commands, enforce approvals |
| `PostToolUse` | After a tool succeeds | Lint files Codex just wrote, run tests |
| `Stop` | When the session ends | Desktop / Slack notification |
| `SessionStart` | When Codex opens the project | Print reminders, project status |
...
```bash
[ -f .Codex/settings.json ] && cat .Codex/settings.json | head -50 || echo "no .Codex/settings.json"
[ -d scripts/hooks ] && ls scripts/hooks/ || echo "no scripts/hooks/ dir"
[ -d .Codex/hooks ] && ls .Codex/hooks/ || echo "no .Codex/hooks/ dir"
...
```
Hooks check:
  .Codex/settings.json:  [exists / not found]
  Existing hooks:         [count or none]
  Detected stack:         [Node / Python / PHP / Go / Rust / Terraform / mixed]
```

If `.Codex/settings.json` already has a `hooks` block, do NOT overwrite — you'll merge new entries in Step 4.
...
Which hooks do you want? (type numbers, comma-separated, or "all")
  ...
  [1] Lint on Write/Edit       — runs your linter after Codex writes a file
  ...
  [3] Run tests on edit        — runs your test suite after Codex edits a file
                                  → npm test / pytest / phpunit / go test / cargo test
                                  → fires: PostToolUse(Edit)
                                  → tip: only enable if test suite is fast (<10s)

  [4] Notify on Stop           — desktop notification (Linux/Mac) when Codex finishes
                                  → fires: Stop
                                  → optional Slack webhook variant available

  [5] Project reminder on start — prints README highlights / TODOs when Codex opens project
                                  → fires: SessionStart
...
**Note:** Codex passes the changed file path via `$CLAUDE_FILE_PATHS` (newline-separated when multiple). The hook script handles that.
...
FILES TO MODIFY:
  .Codex/settings.json   (merging new hooks block — existing config preserved)

.Codex/settings.json hooks block to add:
{
  "hooks": {
    "PreToolUse": [
      {
...
```bash
#!/usr/bin/env bash
# Generated by /hooks — runs linter on files Codex just wrote/edited.
set -e
...
# Returns exit 2 to block; stderr is shown to Codex.
...
```bash
#!/usr/bin/env bash
# Generated by /hooks — runs test suite after Codex edits a file.
set -e
...
```bash
#!/usr/bin/env bash
# Generated by /hooks — notifies when Codex session ends.

MSG="Codex finished in $(basename "$PWD")"
...
```bash
#!/usr/bin/env bash
# Generated by /hooks — prints project reminders when Codex opens.

if [ -f AGENTS.md ]; then
  echo "── AGENTS.md highlights ──"
  head -30 AGENTS.md
fi
...
## Step 7 — Merge into .Codex/settings.json

Read existing `.Codex/settings.json` (if any), merge the new `hooks` block, write back. Never clobber existing keys.

```python
# Pseudocode for merge:
# 1. Read .Codex/settings.json (or {} if missing)
...
✓ Installed N hooks. Test them:
  ...
  1. Block-dangerous:    type "rm -rf /" in Codex → it should block
  2. Lint on write:      ask Codex to edit a file → linter runs after
  3. Notify on stop:     end the session → notification fires

To disable a hook later: edit .Codex/settings.json or delete the script.
To inspect what fired:   tail -f ~/.Codex/logs/hooks.log  (if logging enabled)
```

---

## Reference: How Codex Hooks Work

Documentation: https://docs.Codex.com/en/docs/Codex/hooks

Key concepts:
- Hooks live in `.Codex/settings.json` (project) or `~/.Codex/settings.json` (user-global)
...
- Exit code `2` from a `PreToolUse` hook blocks the tool call. stderr is shown to Codex.
