---
name: budget
description: Set up free-Codex proxy — route Codex and Roo Code traffic to OpenRouter free tier, Ollama local models, NVIDIA NIM, or DeepSeek to save Anthropic credits on non-critical work
version: 1.0.0
author: veekunth217
...
# Budget Mode — Free LLM Routing for Codex

You are a budget-routing specialist. Your job is to set up [free-Codex](https://github.com/Alishahryar1/free-Codex) — a local FastAPI proxy that intercepts Codex (and Roo Code) traffic and routes it to free or cheap LLM providers, while keeping critical work on the paid Anthropic API.
...
```
Without /budget:
  Codex → api.anthropic.com (always paid)
  Roo Code   → api.anthropic.com (always paid)

With /budget:
  Codex → localhost:8082 (proxy) → choose route per task
...
**Important:** This is a community project, not Anthropic-endorsed. Tool support and context limits vary by backend. Use it for bulk work; keep Codex on the official API for architecture decisions and complex debugging.
...
[ -d ~/free-Codex ] && echo "PROXY_DIR=~/free-Codex"
[ -d ~/.local/share/free-Codex ] && echo "PROXY_DIR=~/.local/share/free-Codex"
...
```
Budget mode check:
  free-Codex proxy:  [running on :8082 / not running]
...
```
Budget mode routes Codex traffic to alternative LLMs via a local proxy.
...
Trade-offs you should know:
  ...
  ✗ Thinking blocks not supported on most non-Codex models
...
## Step 2 — Install free-Codex (if not already)

```
Where to install the proxy?
  1. ~/free-Codex (default — easy to find)
  2. ~/.local/share/free-Codex (XDG-compliant)
  3. Custom path
```
...
```bash
# Install uv if missing (Astral's package manager — required by free-Codex)
...
# Clone the proxy
git clone https://github.com/Alishahryar1/free-Codex.git "$PROXY_DIR"
cd "$PROXY_DIR"
cp .env.example .env
```
...
How do you want to run the proxy?
  ...
  2. Background with nohup — keeps running, logs to ~/.local/share/free-Codex.log
...
nohup uv run uvicorn server:app --host 127.0.0.1 --port 8082 \
  > ~/.local/share/free-Codex.log 2>&1 &
echo $! > ~/.local/share/free-Codex.pid
```
...
## Step 5 — Configure Codex + Roo Code Routing
...
```bash
# Toggle by uncommenting these to route Codex through free-Codex
...
```bash
alias Codex-budget='ANTHROPIC_BASE_URL=http://localhost:8082 ANTHROPIC_AUTH_TOKEN=freecc Codex'
alias Codex-paid='unset ANTHROPIC_BASE_URL ANTHROPIC_AUTH_TOKEN; Codex'
```

Now `Codex-budget` runs through the proxy, `Codex` (or `Codex-paid`) hits Anthropic directly.

### For VS Code Codex Extension
...
ROUTING:
  Codex CLI:  alias Codex-budget added to ~/.bashrc
...
USE IT:
  Codex-budget "scaffold 20 react components for the dashboard"
    → routed through proxy → free model → 0 Anthropic tokens

  Codex "audit this auth flow for race conditions"
...
CHECK STATUS:
  ...
  tail -f ~/.local/share/free-Codex.log   # live traffic

STOP THE PROXY:
  kill $(cat ~/.local/share/free-Codex.pid)
```
...
- Repo: https://github.com/Alishahryar1/free-Codex
...
| Tool calls malformed | Switch backend; not all free models support tools well — use Codex direct |
