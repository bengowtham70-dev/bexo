...
`check.mjs` (skill root) is a relation-based audit — run after a build: `node ~/.Codex/skills/taste-saas/check.mjs <project-root>`. Checks viewport lock, x-axis alignment, sticky thead, etc. Warnings advisory, fails are real bugs.
...
1. Read the snippet:
   ```bash
   cat ~/.Codex/skills/taste-saas/audit-overlay.snippet.js
   ```
