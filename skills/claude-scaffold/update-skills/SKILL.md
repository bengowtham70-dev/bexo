---
name: update-skills
description: Pull the latest version of Codex-scaffold-skill (and other installed skills) — shows the changelog, asks before applying, validates after.
version: 1.0.0
author: veekunth217
...
Keeps your `~/.Codex/skills/` directory current. Pulls the latest commits from the upstream repo for `Codex-scaffold-skill` and any other skills you've installed via `git clone`, shows you what changed, and waits for `GO` before applying.
...
```bash
# Find all skill directories under ~/.Codex/skills/
...
Found [N] skill(s) installed via git:
  ...
  ⬆ Updates available:
    [N]  Codex-scaffold-skill   (8 commits behind main)
...
```
─────────────────────────────────────
What's new in Codex-scaffold-skill:
─────────────────────────────────────
...
After pulling, I will:
  □ Run `make validate` on Codex-scaffold-skill (registry sanity check)
...
Options:
  A) Review your edits:  cd ~/.Codex/skills/[skill] && git status
...
✓ Pulled:
  - Codex-scaffold-skill (8 new commits)
...
To activate updated skills in this Codex session:
...
```
No skills found in ~/.Codex/skills/.
Install one first:
  git clone https://github.com/veekunth217/Codex-scaffold-skill.git \
    ~/.Codex/skills/Codex-scaffold-skill
```
...
**Skills installed via Codex plugin marketplace** (e.g. Anthropic document skills via `/plugin install`):
```
ℹ️  These are managed by Codex itself, not by /update-skills.
...
- This skill never modifies skills outside `~/.Codex/skills/`.
- It uses `--ff-only` to avoid surprise merge commits.
- It does NOT delete uninstalled skills automatically — use `rm -rf ~/.Codex/skills/<name>` for that.
