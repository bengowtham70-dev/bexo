---
name: sync
description: Share project memory across devices AND across AI tools (Codex, Cursor, Codex, Roo, Cline, Aider, Windsurf) — git-native, no SaaS, no lock-in.
version: 2.0.0
author: veekunth217
...
1. **Cross-device** — Codex identifies projects by a hash of the absolute path, so context is lost between machines (`/home/alice/myapp` → `-home-alice-myapp` ≠ `/Users/john/myapp` → `-Users-john-myapp`). Solved by `export` / `import`.

2. **Cross-tool** — You use Codex, Cursor, Codex CLI, Roo, Cline, Aider, or Windsurf — each has its own instruction/memory file (`AGENTS.md`, `.cursorrules`, `AGENTS.md`, ...). You re-explain your project to every one. Solved by `tools push` — one canonical memory file → adapter files for every detected tool.

**The shared truth:** `.Codex-context/MEMORY.md` (+ `memory/*.md`), committed to git.
...
Export current project context to `.Codex-context/` and commit.
...
What it does:
1. Calculates the Codex path hash for the current directory
2. Finds `~/.Codex/projects/<hash>/` — reads all `.md` context files
3. Copies them to `.Codex-context/` in the project
4. Creates `.Codex-context/sync-manifest.json` with device metadata
5. Runs `git add .Codex-context/ && git commit -m "context-sync: export <timestamp>"`
...
Restore project context on this device from `.Codex-context/`.
...
What it does:
1. Runs `git pull` to get latest `.Codex-context/`
2. Reads the manifest to confirm what was exported
3. Calculates the path hash for **this device**
4. Creates `~/.Codex/projects/<this-hash>/` if needed
5. Copies all files from `.Codex-context/` to the correct local target
6. Prints a summary of restored files

After import: restart Codex or open the project. Codex immediately has full context.
...
Check manually:
```bash
# What's in the local Codex folder
ls -la ~/.Codex/projects/$(python3 -c "import os; print(os.path.abspath('.').replace('/', '-').replace('\\\\', '-').replace(':', '-'))")/

# What's been exported
ls -la .Codex-context/

# Diff a specific file
diff .Codex-context/MEMORY.md ~/.Codex/projects/$(python3 -c "import os; print(os.path.abspath('.').replace('/', '-').replace('\\\\', '-').replace(':', '-'))")/MEMORY.md
```

Or ask Codex directly to compare them and summarize what's new, changed, or missing.

---

### `/sync clean`

Remove large files from the local Codex project folder to reduce size before export.

Run:
```bash
# Show what's in your Codex project folder
python3 -c "
import os
from pathlib import Path
key = os.path.abspath('.').replace('/', '-').replace('\\\\', '-').replace(':', '-')
folder = Path.home() / '.Codex' / 'projects' / key
if folder.exists():
    for f in sorted(folder.iterdir()):
        print(f'{f.stat().st_size:>10,}  {f.name}')
else:
    print('No Codex project folder found')
"
...
|---|---|---|
| Codex | `AGENTS.md` | always written (universal) |
...
Generate adapter files from `.Codex-context/MEMORY.md` for every detected tool.

Run:
```bash
python scripts/sync-tools.py push            # write them
python scripts/sync-tools.py push --dry-run  # show diff without writing
python scripts/sync-tools.py push --only cursor,Codex  # subset
```
...
3. Writes `.Codex-context/sync-tools-manifest.json` listing what was written
...
- When you update `.Codex-context/MEMORY.md`, re-run push and every adapter regenerates

**Why most adapter files are gitignored:**
They're per-machine projections of the canonical source. Committing them would create merge conflicts every time someone with a different tool stack syncs. The source-of-truth (`.Codex-context/`) is what's committed; the adapters are local rebuilds.

**Phase 2 — live MCP sync (planned, contributions welcome):**
See [PHASE2-MCP.md](PHASE2-MCP.md) for the full spec. Adds a small MCP server alongside the file adapters so edits flow between tools without re-running push. Covers every MCP-capable tool (Codex, Cursor, Codex CLI, Windsurf, Roo, Cline). The file-based Phase 1 stays as the universal fallback and the source of truth.

---

## How the Path Hash Works

Codex generates the project key from the absolute path:
...
source = f"~/.Codex/projects/{claude_key}/"
```
...
These are all the same project but Codex sees three different contexts. `sync-import.py` handles this automatically — it always installs to the correct hash for **the current device**, regardless of where it was exported from.
...
Device A (VPS):
  ...
  2. Work — Codex builds up memory, project context
  3. /sync export → git push

Device B (Mac / laptop):
  1. git clone <repo> (or git pull)
  2. /sync import
  3. Open project in Codex
  4. Codex remembers everything from Device A
...
The `.Codex-context/` folder is already part of the repo (committed). The scripts are in `scripts/`. No dependencies — pure Python stdlib.

Make sure `.Codex-context/` is **not** in `.gitignore` (this is intentional — it's the sync mechanism). The `*.jsonl` exclusion in `.gitignore` prevents conversation history from being committed.
