...
Dispatch multiple subagents (model: `Codex-sonnet-4-6`) — one per search angle. Each subagent:
...
For each solution, dispatch one subagent (model: `Codex-sonnet-4-6`) per source category in parallel:
...
Spawn one subagent (model: `Codex-sonnet-4-6`) per solution — **sequentially, one at a time**. Each subagent independently attempts an implementation or reproduction of its assigned solution, then writes `test_results_<solution_name>.md`. Wait for each subagent to complete before spawning the next.
...
### Step 7 — Decision Summary *(model: `Codex-opus-4-6`)*
